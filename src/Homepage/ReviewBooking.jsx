...
export default function ReviewBooking() {
  ...
  const qrValue = `${window.location.origin}/proceedings?placeId=${encodeURIComponent(id || '')}&roomId=${encodeURIComponent(roomId || '')}&total=${encodeURIComponent(String(grand))}`
  ...
  const handleConfirm = async () => {
    ...
    if (API_BASE && API_BASE.length > 0) {
      try {
        ...
        if (!res.ok) {
          // Try to read response text for debugging (server may return HTML or JSON)
          let bodyText = ''
          try { bodyText = await res.text() } catch (e) { /* ignore */ }
          console.error('Server returned non-OK status', res.status, bodyText)
          // Continue to fallback to localStorage without throwing an uncaught object
        }
        ...
        try {
          ...
          console.error('Failed to persist booking locally', err2)
        }
      }
    } else {
    ...
    try {
      localStorage.removeItem(`order:${id}:${roomId}`)
    } catch {
      // ignore
    }
        ...
        <div className="booking-info">
          ...
          {(checkin || travelers) && (
            <div className="booking-context">
              ...
              {travelers ? (
                <div>
                  Travelers: <strong>{travelers}</strong>
                </div>
              ) : null}
            </div>
          )}
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <button
              className="add-food"
              onClick={() =>
                navigate(`/place/${id}/homestays/book/${roomId}/food${window.location.search}`)
              }
            >
              Edit Food
            </button>
          ...
          <div className="proceed-wrap">
            <button className="proceed-btn" onClick={() => setConfirmOpen(true)}>
              Book Now
            </button>
          </div>
        </div>
      ...
      {confirmOpen && (
        ...
        >
          ...
          >
            ...
            <p>
              Your reservation has been confirmed. Total paid: <strong>${grand}</strong>
            </p>
            <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:12, marginTop:12}}>
              <div style={{background:'#f7f9fb', padding:12, borderRadius:8}}>
                <div style={{marginBottom:8, fontSize:13}}>Scan to proceed (QR)</div>
                <QRCode value={qrValue} size={128} />
              </div>

              <div style={{display:'flex', gap:8, justifyContent:'center', width:'100%'}}>
                <button className="proceed-btn" onClick={() => { setConfirmOpen(false); navigate(`/proceedings?placeId=${encodeURIComponent(id)}&roomId=${encodeURIComponent(roomId)}&total=${encodeURIComponent(String(grand))}`) }}>
                  Proceed
                </button>
                ...
                <button className="add-food" onClick={() => setConfirmOpen(false)}>
                  Close
                </button>
              </div>
            </div>
